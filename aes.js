...
const decrypt = require('./dencrypt');

const encryptAnddecrypt = async(file,password) =>{

  const eny = await encrypt(file,password);
  if(eny)
  {
    await decrypt(file+'.enc','output.txt',password);
  }
};
...
encryptAnddecrypt(file,password);
